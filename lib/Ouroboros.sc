	...
	*new {
	...
	init {
		...
		SynthDef("defRecord",{
			...
			t_record=0,threshold=60.neg,duration=1;
		...
		oscRecordInfo = OSCFunc({ |msg| 
			var id=msg[3];
			dataRecord.put(id,[msg[4],msg[5],msg[6]]);
		}, '/ouroborousRecordInfo');
	...
	recordStart {
		arg argID;
		if (synRecord.at(argID).notNil,{
			if (synRecord.at(argID).isRunning,{
				// force recording and set predelay to 0
				preDelay=0;
				synRecord.at(argID).set(\t_record,1);
			});
		});
	}
	...
	record {
		...
		Buffer.alloc(server,server.sampleRate*180,2,{
			...
			synRecord.put(id,Synth.tail(group,"defRecord",
				...
				\t_record,argStart,\duration,argSeconds,
